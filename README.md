...
> This started as a final year project for the BSSE Degree that I was/am pursuing, now aiming to make this more than that.
...
- The admin then will check if the registration information (blockchain account address, name, and NID_NUM number) is valid and matches with his record. If yes, then the admin approves the registered user making them eligible to take part and cast their respective vote in the election.
...
1. Clone this repository
   ...
   ```shell
   git clone https://github.com/Anwarkabir/dVoting.git
   cd dVoting
   ```
...
- [ ] **Email Verification**—adding email/NID_NUM verification (OTP, etc..) while registering for voters.
- [ ] **Automated Verification**—adding an automated verification (rather than manually approving by the admin) for the registered users. This could be based on the custom cooperation email, custom list of emails, or custom list of NID_NUM numbers, etc.
