...
export default class Registration extends Component {
  ...
  updateVoterNID_NUM = (event) => {
    this.setState({ voterNID_NUM: event.target.value });
  };
  registerAsVoter = async () => {
    await this.state.ElectionInstance.methods
      .registerAsVoter(this.state.voterName, this.state.voterNID_NUM)
            ...
            <div className="container-main">
              ...
              <div className="container-item">
                <form>
                  ...
                  <div className="div-li">
                    <label className={"label-r"}>
                      NID_NUM <span style={{ color: "tomato" }}>*</span>
                      <input
                        className={"input-r"}
                        type="number"
                        placeholder="eg. 9841234567"
                        value={this.state.voterNID_NUM}
                        onChange={this.updateVoterNID_NUM}
                      />
                    </label>
                  </div>
                  <p className="note">
                    <span style={{ color: "tomato" }}> Note: </span>
                    <br /> Make sure your account address and NID_NUM number are
                    correct. <br /> Admin might not approve your account if the
                    provided NID_NUM number nub does not matches the account
                    address registered in admins catalogue.
                  </p>
                  <button
                    className="btn-add"
                    disabled={
                      this.state.voterNID_NUM.length !== 10 ||
